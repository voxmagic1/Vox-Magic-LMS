...
backend:
  - task: "Student password management (change, forced first-login, forgot/reset, admin reset)"
    implemented: true
    working: true
    file: "app/api/[[...path]]/route.js"
    stuck_count: 0
    ...
    status_history:
        -working: "NA"
        -agent: "main"
        -comment: "NEW student self-service password features. Endpoints: (1) POST /api/students/change-password (auth student; requires currentPassword+newPassword; wrong current->401; newPassword<8->400; success clears mustResetPassword). (2) POST /api/students/first-password (auth student; only works while mustResetPassword=true else 400; sets pw & clears flag). (3) POST /api/auth/forgot-password ({email}; ALWAYS returns generic 200 {ok:true} to prevent email enumeration; if student exists creates token in password_resets collection (60min expiry) and sends Resend email). (4) POST /api/auth/reset-password ({token,newPassword}; invalid/expired/used token->400; newPassword<8->400; success updates pw, clears mustResetPassword, marks token used, invalidates student sessions). (5) POST /api/admin/students/reset-password (auth staff; {studentId,newPassword}; sets student pw + mustResetPassword=true + kills student sessions; audit logged as student_password_reset). Seed test student lms.tester@voxmagic.test/Test1234 now has mustResetPassword=false so normal login works."
        -working: true
        -agent: "testing"
        -comment: "✅ ALL 5 TEST SUITES PASSED (100% success rate). Comprehensive testing of all student password management endpoints completed successfully. Test results: (1) POST /api/students/change-password: ✅ Missing currentPassword or newPassword correctly returns 400, ✅ Wrong currentPassword correctly returns 401, ✅ newPassword < 8 chars correctly returns 400, ✅ Valid password change from 'Test1234' to 'StudentNew123' returns 200 {ok:true, message:'Password changed successfully'}, ✅ Login with new password 'StudentNew123' successful (verified new password works), ✅ Password changed back to 'Test1234' successfully (defaults preserved). (2) POST /api/auth/forgot-password: ✅ Missing email correctly returns 400, ✅ Valid email lms.tester@voxmagic.test returns 200 {ok:true, message:'If an account exists for that email, a reset link has been sent.'} (generic message to prevent enumeration), ✅ Verified password_resets doc created in MongoDB with token, used=false, expiresAt ~60min from now, ✅ Non-existent email 'nobody-doesnotexist@example.com' ALSO returns 200 {ok:true} (correct - no email enumeration), ✅ Verified NO password_resets doc created for non-existent email (correct). (3) POST /api/auth/reset-password: ✅ Invalid token 'garbage' correctly returns 400, ✅ newPassword < 8 chars correctly returns 400, ✅ HAPPY PATH: Valid token with newPassword 'ResetFlow123' returns 200 {ok:true, message:'Password reset successfully. You can now log in with your new password.'}, ✅ Reusing same token correctly returns 400 (token now marked used), ✅ Student can log in with reset password 'ResetFlow123' (verified), ✅ Password changed back to 'Test1234' via change-password (defaults preserved). (4) POST /api/admin/students/reset-password: ✅ Admin login successful (admin@voxmagic.test/Admin1234), ✅ Retrieved student ID for lms.tester@voxmagic.test from GET /api/admin/students, ✅ Missing studentId or newPassword correctly returns 400, ✅ newPassword < 8 chars correctly returns 400, ✅ Unknown studentId correctly returns 404, ✅ Valid reset with newPassword 'AdminSet123' returns 200 {ok:true, message:'Password reset for LMS Tester. They must set a new password on next login.'}, ✅ Verified in MongoDB: student's mustResetPassword=true (correct), ✅ Verified in MongoDB: all prior student sessions removed (count: 0), ✅ Verified GET /api/admin/audit-logs contains 'student_password_reset' action entry with correct metadata. (5) FINAL CLEANUP: ✅ Ran 'node /app/scripts/seed_test_student.js' successfully, ✅ Admin login with admin@voxmagic.test/Admin1234 working (returns token), ✅ Student login with lms.tester@voxmagic.test/Test1234 working (returns token and student.mustResetPassword=false). All authentication, authorization, validation, password change flows, forgot/reset flows, admin reset flows, session invalidation, audit logging, email enumeration prevention, and cleanup working correctly. The student password management API is PRODUCTION-READY with no critical issues found."
...
agent_communication:
    -agent: "main"
    -message: "Please backend-test the NEW student password features. Student login POST /api/auth/login {email:'lms.tester@voxmagic.test',password:'Test1234'} returns {token, student}; use Bearer token. Test: (1) POST /api/students/change-password: missing fields->400; wrong currentPassword->401; newPassword<8->400; correct current + valid new->200 ok (IMPORTANT: this changes the password, so afterwards change it back to Test1234 using the new password, OR re-run node /app/scripts/seed_test_student.js at the end). (2) POST /api/auth/forgot-password with a real student email -> 200 {ok:true} generic; with a non-existent email -> ALSO 200 {ok:true} (no enumeration). Note: actual email send depends on Resend config; a password_resets doc should be created for the existing student. (3) POST /api/auth/reset-password: bad/random token -> 400; newPassword<8 -> 400. To test the happy path, you may read the created token from the password_resets collection (Mongo) for the student and call reset-password with it -> 200 ok; then the token is 'used' so calling again -> 400. (4) POST /api/admin/students/reset-password (admin token admin@voxmagic.test/Admin1234, but that admin has mustResetPassword=true; login still yields a working token; if admin/me flow blocks you, first set admin password via /api/admin/profile/first-password OR just use the token from admin login which works regardless). Provide studentId of lms.tester (from GET /api/admin/students). Expect 200; missing fields->400; pw<8->400; unknown studentId->404. After it runs, the student's mustResetPassword becomes true and sessions are invalidated. (5) After all tests, RE-SEED to restore defaults: node /app/scripts/seed_test_student.js (restores admin@voxmagic.test/Admin1234 mustResetPassword=true, and lms.tester@voxmagic.test/Test1234 mustResetPassword=false). Report final credential state. Do NOT complete any real external Flutterwave charge."
agent_communication:
    -agent: "testing"
    -message: "✅ STUDENT PASSWORD MANAGEMENT BACKEND TESTING COMPLETE - ALL 5 TEST SUITES PASSED (100% success rate). Comprehensive testing of all student password management endpoints completed successfully. Test results: (1) POST /api/students/change-password: ✅ Missing currentPassword or newPassword correctly returns 400, ✅ Wrong currentPassword correctly returns 401, ✅ newPassword < 8 chars correctly returns 400, ✅ Valid password change from 'Test1234' to 'StudentNew123' returns 200 {ok:true, message:'Password changed successfully'}, ✅ Login with new password 'StudentNew123' successful (verified new password works), ✅ Password changed back to 'Test1234' successfully (defaults preserved). (2) POST /api/auth/forgot-password: ✅ Missing email correctly returns 400, ✅ Valid email lms.tester@voxmagic.test returns 200 {ok:true, message:'If an account exists for that email, a reset link has been sent.'} (generic message to prevent enumeration), ✅ Verified password_resets doc created in MongoDB with token, used=false, expiresAt ~60min from now, ✅ Non-existent email 'nobody-doesnotexist@example.com' ALSO returns 200 {ok:true} (correct - no email enumeration), ✅ Verified NO password_resets doc created for non-existent email (correct). (3) POST /api/auth/reset-password: ✅ Invalid token 'garbage' correctly returns 400, ✅ newPassword < 8 chars correctly returns 400, ✅ HAPPY PATH: Valid token with newPassword 'ResetFlow123' returns 200 {ok:true, message:'Password reset successfully. You can now log in with your new password.'}, ✅ Reusing same token correctly returns 400 (token now marked used), ✅ Student can log in with reset password 'ResetFlow123' (verified), ✅ Password changed back to 'Test1234' via change-password (defaults preserved). (4) POST /api/admin/students/reset-password: ✅ Admin login successful (admin@voxmagic.test/Admin1234), ✅ Retrieved student ID for lms.tester@voxmagic.test from GET /api/admin/students, ✅ Missing studentId or newPassword correctly returns 400, ✅ newPassword < 8 chars correctly returns 400, ✅ Unknown studentId correctly returns 404, ✅ Valid reset with newPassword 'AdminSet123' returns 200 {ok:true, message:'Password reset for LMS Tester. They must set a new password on next login.'}, ✅ Verified in MongoDB: student's mustResetPassword=true (correct), ✅ Verified in MongoDB: all prior student sessions removed (count: 0), ✅ Verified GET /api/admin/audit-logs contains 'student_password_reset' action entry with correct metadata. (5) FINAL CLEANUP: ✅ Ran 'node /app/scripts/seed_test_student.js' successfully, ✅ Admin login with admin@voxmagic.test/Admin1234 working (returns token), ✅ Student login with lms.tester@voxmagic.test/Test1234 working (returns token and student.mustResetPassword=false). All authentication, authorization, validation, password change flows, forgot/reset flows, admin reset flows, session invalidation, audit logging, email enumeration prevention, and cleanup working correctly. The student password management API is PRODUCTION-READY with no critical issues found."
